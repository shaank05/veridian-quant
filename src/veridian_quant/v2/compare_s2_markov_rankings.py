"""Run S2 Markov candidate ranking experiments and write a summary."""
...
DEFAULT_RANKINGS = S2_CANDIDATE_RANKING_MODES
...
def main(argv: Iterable[str] | None = None) -> int:
    """Run each requested S2 candidate ranking mode and compare results."""
    ...
    reporter.info("Loading OHLCV data for S2 Markov ranking comparison...")
    ...
    summary_rows = []
    for ranking_mode in args.rankings:
        reporter.info(f"Running S2 Markov ranking experiment: {ranking_mode}")
        ...
        result = run_s2_markov_portfolio_backtest(
            ...
            s2_candidate_ranking_mode=ranking_mode,
        )
        export_portfolio_backtest_csvs(
            ...
            progress_reporter=reporter,
            include_all_signal_diagnostics=args.include_all_signal_diagnostics,
        )
    ...
    pd.DataFrame(summary_rows, columns=SUMMARY_COLUMNS).to_csv(
        output_dir / "s2_markov_ranking_experiment_summary.csv",
        index=False,
    )
    reporter.complete(
        f"Completed S2 Markov ranking comparison. Wrote reports to {output_dir}"
    )
    return 0
...
def _summary_row(
    ...
    return {
        "markov_signal_filter": markov_signal_filter,
        "s2_candidate_ranking_mode": ranking_mode,
...
def _parse_args(argv: Iterable[str] | None) -> Namespace:
    parser = ArgumentParser(
        description="Compare S2 Markov candidate ranking experiments."
    )
...
def _parse_rankings(value: str) -> tuple[str, ...]:
    rankings = tuple(item.strip() for item in value.split(",") if item.strip())
    invalid = [item for item in rankings if item not in S2_CANDIDATE_RANKING_MODES]
    if invalid:
        raise ArgumentTypeError(
            f"unsupported ranking(s): {', '.join(invalid)}; "
