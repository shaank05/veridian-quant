...
| S2 full state x risk / in-trade audit design | Phase 36J docs-only design | No | Design only | Full read-only Lane A/Lane B audit helper specified after reconstruction passed with caveats; next gate is 36K implementation; no dynamic exit, filter, risk rule, backtest, or strategy change approved |
