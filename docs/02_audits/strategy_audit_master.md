...
- S2 full state x risk / in-trade audit design: `docs/03_research/s2_state_risk_intrade_full_audit_design.md`
...
| S2 full state x risk / in-trade audit design | Phase 36J docs-only design | No | Design only | Full read-only Lane A/Lane B audit helper specified after reconstruction passed with caveats; no dynamic exit, filter, risk rule, backtest, or strategy change approved |
| S2 full state x risk / in-trade audit helper | Phase 36K read-only helper implementation | Yes | Not run yet | Helper and CLI implemented for future controlled export; next gate is 36L audit run; no dynamic exit, filter, risk rule, backtest, or strategy change approved |
